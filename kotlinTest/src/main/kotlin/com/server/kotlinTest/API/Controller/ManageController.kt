...
import com.server.kotlinTest.Service.KafkaService
...
class ManageController {
    ...
    fun CreateUserapi(request: UserRequest?): UserResponse {
        ...
        val username = request.username ?: throw C_ManageException.UsernameisNull()
        val userpassword = request.userpassword ?: throw C_ManageException.UserpasswordisNull()
        val email = request.email ?:throw  C_ManageException.UseremailisNull()

        kafkaService.sendMessage(EmailSender(email,"welcome"))
        return usermapper.Userapi2Response(userapiService.CreateUserapi(usercode, username, userpassword,email))
    }
